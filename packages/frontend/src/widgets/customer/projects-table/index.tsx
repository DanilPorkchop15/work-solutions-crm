...
import { ProjectPreview, ProjectsTableModule, useProjectsTableModule } from "@frontend/entities/project";
...
export const CustomerProjectsTableWidget: React.FC<TableProps<ProjectPreview>> = observer(
  function CustomerProjectsTable(props: TableProps<ProjectPreview>) {
    const projectsTableModule: ProjectsTableModule = useProjectsTableModule();
    ...
    const applySearchAndFilter: (data: ProjectPreview[]) => ProjectPreview[] = (data: ProjectPreview[]) =>
      data
        ...
        .filter(project => project.customer.id === id);
